...
INSERT INTO Recipe_Ingredients
  ...
  SELECT A.RecipeTitle
		,C.IngredientName
into TB 
  FROM [Case_Study_1].[dbo].[Recipes] A  
		JOIN [Case_Study_1].[dbo].Recipe_Ingredients B on A.RecipeID = B.RecipeID
		JOIN Ingredients C on B.IngredientID = C.IngredientID
	
SELECT * 
FROM
(SELECT *
FROM TB
WHERE IngredientName = 'Beef' or IngredientName = 'Garlic') A
 JOIN
(SELECT RecipeTitle, CountOfIngre = COUNT(IngredientName)
FROM TB
GROUP BY RecipeTitle
HAVING COUNT(IngredientName) = 2) B
ON A.RecipeTitle = B.RecipeTitle

----2. HOW many Recipes have only ONE KIND OF INGREDIENTS?

SELECT COUNT(*)
FROM (SELECT RecipeTitle, No_of_IngredientClass = count(d.IngredientClassID) 

FROM [Case_Study_1].[dbo].[Recipes] A  
		JOIN [Case_Study_1].[dbo].Recipe_Ingredients B on A.RecipeID = B.RecipeID
		JOIN Ingredients C on B.IngredientID = C.IngredientID
		join Ingredient_Classes D on c.IngredientClassID = d.IngredientClassID
group by RecipeTitle
having count(d.IngredientClassID) = 1
) a

----3. HOW MANY INGREDIENTS THAT HAVENT BEEN USED IN ANY RECIPES?

SELECT COUNT(*) AS NO_USED_INGRE 
FROM
(SELECT A.IngredientName, COUNT(C.RecipeTitle) AS COUNT_RECIPES

FROM Ingredients A 
LEFT JOIN Recipe_Ingredients B ON A.IngredientID = B.IngredientID
LEFT JOIN Recipes C ON B.RecipeID = C.RecipeID
GROUP BY A.IngredientName
HAVING COUNT(C.RecipeTitle) = 0) X

---4. STATE GROUP OF 2 OF RECIPES SHARING AT LEAST 3 OF THE SAME INGREDIENTS
SELECT X.RecipeTitle AS Recipe1, y.RecipeTitle as Recipe2

FROM 
(SELECT a.RecipeTitle
	, Ingre1 = a.IngredientName
	, Ingre2 = b.IngredientName
	, Ingre3 = c.IngredientName
FROM TB a join TB b on a.RecipeTitle = b.RecipeTitle and a.IngredientName <> b.IngredientName
		join tb c on a.RecipeTitle = c.RecipeTitle and a.IngredientName <> c.IngredientName and b.IngredientName <> c.IngredientName) X
...
(SELECT a.RecipeTitle
	...
	, Ingre3 = c.IngredientName
FROM TB a join TB b on a.RecipeTitle = b.RecipeTitle and a.IngredientName <> b.IngredientName
		join tb c on a.RecipeTitle = c.RecipeTitle and a.IngredientName <> c.IngredientName and b.IngredientName <> c.IngredientName) Y

ON X.RecipeTitle > Y.RecipeTitle  AND X.Ingre1 = Y.Ingre1 AND X.Ingre2 = Y.Ingre2 AND X.Ingre3 = Y.Ingre3
group by x.RecipeTitle, y.RecipeTitle;

---5. WHICH RECIPES USE THE MOST GARLIC GLOVES?
WITH Garlic as (SELECT a.RecipeTitle,c.IngredientName, b.Amount, d.MeasurementDescription
  FROM [Case_Study_1].[dbo].[Recipes] A  
		JOIN [Case_Study_1].[dbo].Recipe_Ingredients B on A.RecipeID = B.RecipeID
		JOIN Ingredients C on B.IngredientID = C.IngredientID
		JOIN Measurements d on d.MeasureAmountID = c.MeasureAmountID
where c.IngredientName = 'Garlic')

SELECT RecipeTitle, IngredientName, Amount, MeasurementDescription
FROM Garlic
WHERE Amount = (Select max(amount) from Garlic);